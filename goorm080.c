...
TreeNode *search(TreeNode *root, element key) // 이진탐색트리 탐색함수
{
	TreeNode *p = root;
	while (p != NULL)
	{
...
TreeNode *insert_node(TreeNode *node, element key) // 순환적으로 트리를 내려가서 삽입
{
	// 트리가 공백이면 새로운 노드를 반환
	if (node == NULL)
		return new_node(key);
...
int main()
{
	...
	do
	{
		...
		case 'i':
			printf("단어:");
			gets_s(e.word, 100); // 단어 한줄 e.word에 입력받음
			printf("의미:");
			gets_s(e.meaning, 200);		  // 의미 한줄 e.meaning에 입력받음
